                          ...
                          <div style={{ ...S.configCard, padding: 0, overflow: "auto" }}>
                            ...
                            {tooltipCol && (() => {
                              const tips: Record<string, { title: string; desc: string }> = {
                                ...
                                R: { title: "R \u2014 Relevan\u021B\u0103 (Relevance)", desc: `Scor mediu 1\u201310. M\u0103soar\u0103 c\u00e2t de relevant este mesajul pentru publicul \u021Bint\u0103. R este cheia de contact a formulei (~10% din magnitudinea C, dar 100% condi\u021Bie de activare). Sub pragul de ${GATE}, audien\u021Ba se dezangajeaz\u0103 \u0219i CTA scade ~58%, indiferent de I \u0219i F.` },
        ...
        {activeTab === "interpretare" && (() => {
          ...
          const getConclusion = (pct: number, avgR: number, zoneMatch: boolean): string => {
            ...
            const rRole = gateOk
              ...
              : `R = ${avgR.toFixed(1)} (sub Gate ${GATE}) — cheia de contact nu este activata. I×F (motorul) tot produce un scor matematic, dar audienta se dezangajeaza: CTA scade ~58% (de la ~6.55 la ~2.75).`;
            if (pct >= 80 && gateOk && zoneMatch) {
          ...
          const getInterpretation = (key: string, val: string, ctx?: Record<string, unknown>): { sections: { heading: string; text: string }[] } => {
            ...
            switch (key) {
              case "hypothesis_total": {
                ...
                { heading: "Relevance Gate", text: `Din ${_gateTotal} materiale, ${_gatePass} trec Gate-ul (R >= ${GATE}) — rata de ${_gateRate}%. Materialele cu R < ${GATE} sunt sub Gate — audienta se dezangajeaza si CTA scade ~58%, indiferent de calitatea I×F. ${_gateRate >= 80 ? "Majoritatea materialelor sunt relevante pentru audienta." : _gateRate >= 50 ? "Jumatate din materiale nu trec pragul de relevanta — formula e penalizata de targetare." : "Majoritatea materialelor nu sunt relevante — problema de targetare, nu de formula."}` },
              ...
              case "score_r": return { sections: [
                ...
                { heading: "Cum se aplica in formula", text: `In formula R+(I×F)=C, R este cheia de contact (~10% din magnitudinea C), iar I×F este motorul (~90%). R se aduna la produsul I×F: ${val} + ${_ctx.cf && _ctx.r ? (Number(_ctx.cf) - Number(_ctx.r)).toFixed(1) : "?"} = ${_ctx.cf || "?"}. Daca R < ${GATE}, audienta se dezangajeaza — CTA scade ~58% indiferent de calitatea I×F. R nu "opreste" formula matematic, dar fara activare, motorul I×F merge in gol.` },
                { heading: "Relevance Gate", text: `Pragul Gate (R >= ${GATE}) este conditia de activare: R nu opreste formula matematic, dar sub Gate audienta se dezangajeaza si CTA scade ~58% (de la ~6.55 la ~2.75). Analogie: o reclama la anvelope pentru cineva fara masina — motorul I×F merge (poate fi spectaculoasa si interesanta), dar audienta ignora mesajul. Cu R = ${val}, ${v >= GATE ? `materialul TRECE Gate-ul — cheia de contact e activata, motorul I×F produce claritate actionabila.` : `materialul NU trece Gate-ul — audienta se dezangajeaza inainte ca I×F sa conteze. Prioritate: imbunatatirea relevantei.`}` },
                { heading: "De ce arata acest rezultat", text: v >= 7 ? `R = ${val} indica o relevanta ridicata. Respondentii considera ca mesajul se adreseaza direct nevoilor lor. Aceasta baza solida permite amplificarea maxima prin I×F.` : v >= 5 ? `R = ${val} indica o relevanta moderata. Mesajul trece Gate-ul (>= ${GATE}) si rezoneza partial, dar exista spatiu de imbunatatire a targetarii.` : v >= GATE ? `R = ${val} indica relevanta la limita. Mesajul abia trece Gate-ul — functiona, dar impactul formulei va fi limitat de baza slaba de relevanta.` : `R = ${val} indica relevanta scazuta, sub pragul Gate de ${GATE}. Mesajul nu rezoneza cu audienta tinta, ceea ce anuleaza eficacitatea chiar si a celui mai bun I×F.` },
                { heading: "Lantul cauzal R → C → CTA", text: `R este cheia de contact (~10% din C ca magnitudine, dar 100% conditie de activare). Lantul: R activeaza → I×F (motorul, ~90%) produce Claritate (C) → C coreleaza cu CTA (confirmat H2) → Brand modereaza C→CTA (testat H3). IMPORTANT: C = Claritate, nu actiune directa. CTA este un pas separat. Sub Gate (R<${GATE}), CTA scade ~58% — nu pentru ca formula "nu functioneaza", ci pentru ca audienta se dezangajeaza. Cu R = ${val}, ${v >= GATE ? "cheia e activata — I×F produce claritate actionabila" : "cheia NU e activata — audienta se dezangajeaza indiferent de I×F"}.` },
                { heading: "Concluzie", text: `Cu R = ${val}, ${v >= GATE ? `materialul depaseste Relevance Gate si poate beneficia de amplificarea I×F pentru a genera claritate si actiune (CTA).` : `materialul nu depaseste Relevance Gate — prioritatea este imbunatatirea relevantei inainte de a optimiza Interesul sau Forma.`}` },
              ]};
              case "score_i": return { sections: [
                ...
                { heading: "Cum se aplica in formula", text: `In formula R+(I×F)=C, I este primul factor al componentei multiplicative. Interesul se inmulteste cu Forma (F) — daca continutul este interesant DAR prost executat, produsul I×F va fi modest. Daca este si interesant SI bine executat, amplificarea este maxima. Cu I = ${val} si F = ${_ctx.f || "?"}, produsul I×F = ${_ctx.i && _ctx.f ? (Number(_ctx.i) * Number(_ctx.f)).toFixed(1) : "?"}.` },
                { heading: "De ce arata acest rezultat", text: v >= 7 ? `I = ${val} indica un interes puternic. Continutul, mesajul sau povestea rezoneza puternic cu audienta, genereaza curiozitate si implicare emotionala.` : v >= 5 ? `I = ${val} indica un interes moderat. Materialul capteaza atentia dar nu iese in evidenta — exista potential de imbunatatire prin storytelling mai puternic, hooks de atentie sau elemente de noutate.` : `I = ${val} indica un interes scazut. Materialul nu reuseste sa capteze atentia — continutul este perceput ca generic, repetitiv sau fara valoare adaugata.` },
                { heading: "Cum se interpreteaza", text: `I >= 7: Interes puternic — amplificarea prin F va fi maxima. I 5-7: Interes moderat — produsul I×F va fi mediu, exista potential de crestere. I < 5: Interes sub medie — chiar si o executie excelenta (F mare) nu compenseaza un continut care nu capteaza.` },
                { heading: "Lantul cauzal I×F → C → CTA", text: `I×F este motorul formulei (~90% din C). I amplifica F multiplicativ: I ridicat + F ridicat = sinergie maxima. Lantul: I×F produce Claritate (C) → C coreleaza cu CTA (confirmat H2) → Brand modereaza C→CTA (H3). IMPORTANT: C = Claritate, nu actiune directa. CTA e un pas separat, influentat de C dar si de Brand si context. Cu I = ${val}, contributia la motorul I×F este ${v >= 7 ? "puternica — continut captivant care amplifica maxim Forma" : v >= 5 ? "moderata — continutul rezoneza partial" : "limitata — continutul nu capteaza atentia"}. Dar fara R >= ${GATE} (cheia de contact), motorul merge in gol.` },
                { heading: "Concluzie", text: `Cu I = ${val}, ${v >= 7 ? "materialul are un continut captivant care amplifica puternic impactul formulei si creste probabilitatea de actiune (CTA)." : v >= 5 ? "materialul mentine atentia dar nu iese in evidenta — optimizarea continutului (storytelling, hooks) poate creste semnificativ scorul C si rata de conversie." : "materialul necesita o reformulare fundamentala a continutului pentru a genera interes si, implicit, claritate si actiune."}` },
              ]};
              case "score_f": return { sections: [
                ...
                { heading: "Cum se aplica in formula", text: `In formula R+(I×F)=C, F este al doilea factor al componentei multiplicative. Forma amplifica (sau diminueaza) Interesul: un continut interesant (I mare) cu executie excelenta (F mare) produce un produs I×F maxim. Forma slaba "franeaza" impactul chiar si al celui mai bun continut. Cu I = ${_ctx.i || "?"} si F = ${val}, produsul I×F = ${_ctx.i ? (Number(_ctx.i) * v).toFixed(1) : "?"}.` },
                { heading: "De ce arata acest rezultat", text: v >= 7 ? `F = ${val} indica o executie de inalta calitate. Designul, layoutul, tipografia, culorile si elementele vizuale sunt percepute ca profesionale si coerente.` : v >= 5 ? `F = ${val} indica o executie acceptabila. Nu sunt erori grave, dar materialul nu impresioneza — poate fi imbunatatit prin design mai atent, imagine mai clara, sau layout mai profesional.` : `F = ${val} indica o executie slaba. Materialul este perceput ca neprofesional, cu probleme vizuale, text greu de citit, sau design necorespunzator.` },
                { heading: "Cum se interpreteaza", text: `F >= 7: Executie profesionala — amplifica puternic Interesul. F 5-7: Executie acceptabila — nu obstructioneaza, dar nu impresioneza. F < 5: Executie sub medie — diminueaza impactul indiferent de calitatea continutului.` },
                { heading: "Lantul cauzal I×F → C → CTA", text: `F amplifica I in motorul I×F (~90% din C). Un design profesional face mesajul mai usor de procesat, crescand Claritatea (C). Lantul: I×F produce C → C coreleaza cu CTA (H2) → Brand modereaza C→CTA (H3). C = Claritate, nu actiune directa — CTA e un pas separat. Cu F = ${val}, amplificarea executiei este ${v >= 7 ? "puternica — designul profesional sustine claritatea" : v >= 5 ? "moderata — executia nu obstructioneaza dar nici nu amplifica" : "slaba — designul deficitar poate confuza mesajul chiar daca continutul e bun"}. Dar fara R >= ${GATE} (cheia de contact), motorul merge in gol.` },
                { heading: "Concluzie", text: `Cu F = ${val}, ${v >= 7 ? "calitatea executiei amplifica puternic impactul continutului in formula I×F si sustine claritatea mesajului." : v >= 5 ? "executia este acceptabila dar poate fi optimizata pentru a creste multiplicatorul I×F si, implicit, claritatea perceputa." : "executia necesita imbunatatiri semnificative — este cel mai rapid mod de a creste scorul C si rata de conversie."}` },
              ]};
              case "score_cf": return { sections: [
                { heading: "Ce reprezinta C formula", text: `Scorul C formula = ${val} este rezultatul matematic al formulei RIFC: R+(I×F)=C. Acest scor prezice cat de clar si convingator ar trebui sa fie mesajul, pe baza celor trei componente masurate (R, I, F). Scala este 0-110 (R max 10 + I max 10 × F max 10 = 110).` },
                { heading: "Cum se calculeaza", text: `R+(I×F)=C concret: ${_ctx.r || "?"} + (${_ctx.i || "?"} × ${_ctx.f || "?"}) = ${_ctx.r || "?"} + ${_ctx.i && _ctx.f ? (Number(_ctx.i) * Number(_ctx.f)).toFixed(1) : "?"} = ${val}. Normalizat pe scala 1-10: Cf/11 = ${_ctx.cf ? (Number(_ctx.cf) / 11).toFixed(2) : "?"}. Aceasta valoare normalizata se compara cu C perceput (${_ctx.cp || "?"}).` },
                ...
                { heading: "C = Claritate (nu CTA direct)", text: `C (Clarity) = cat de clar este mesajul. IMPORTANT: C nu este CTA. Lantul cauzal: R (cheia, ~10%) activeaza → I×F (motorul, ~90%) produce C (Claritate) → C coreleaza cu CTA (confirmat H2, Pearson) → Brand modereaza C→CTA (H3). Formula subestimeaza sistematic (Delta pozitiv) — oamenii percep mai multa claritate decat prezice formula, din cauza brandului, experientei si contextului. Cu Cf = ${val} (normalizat ${_ctx.cf ? (Number(_ctx.cf) / 11).toFixed(2) : "?"}/10), formula prezice o claritate ${v > 80 ? "ridicata — baza puternica pentru actiune" : v > 50 ? "moderata — actiunea depinde si de Brand si context" : "scazuta — baza insuficienta pentru actiune"}.` },
              ...
              case "score_cp": return { sections: [
                ...
                { heading: "Rolul in validarea formulei", text: `C perceput este ancora de validare a formulei RIFC. Formula prezice Cf = ${_ctx.cf || "?"} (normalizat ${_ctx.cf ? normCf(Number(_ctx.cf)) : "?"}/10). C perceput = ${val}/10. Daca sunt apropiate, formula prezice corect; daca difera, formula nu surprinde toti factorii. Delta actuala = ${_ctx.delta || "?"}.` },
                { heading: "De ce arata acest rezultat", text: `C perceput = ${val} reflecta experienta subiectiva a respondentului cu materialul. ${v > 8 ? "Respondentul percepe mesajul ca foarte clar si convingator — impactul real este puternic." : v > 6 ? "Respondentul percepe mesajul ca destul de clar — claritate buna, cu spatiu de optimizare." : v > 5 ? "Respondentul percepe mesajul ca moderat de clar — exista loc semnificativ de imbunatatire." : "Respondentul nu a fost convins de claritatea mesajului — materialul nu comunica eficient."}` },
                { heading: "C = Claritate → CTA", text: `C perceput este masura directa a claritatii reale a mesajului. In modelul RIFC, claritatea este scopul final al formulei: daca mesajul e clar, respondentul actioneaza (CTA — Call to Action). Cu Cp = ${val}, respondentii percep claritatea ca ${v > 8 ? "foarte ridicata — probabilitate mare de actiune/cumparare" : v > 6 ? "buna — claritatea sustine actiunea, dar nu maximal" : v > 5 ? "moderata — o parte din audienta va actiona, alta nu" : "scazuta — probabilitate redusa de actiune, mesajul nu convinge"}. Brand-ul poate modera aceasta relatie: un brand puternic creste probabilitatea de actiune chiar si cu claritate moderata (testat in H3).` },
                { heading: "Directia discrepantei", text: `${_ctx.cf ? `Cf normalizat = ${normCf(Number(_ctx.cf))}, Cp = ${val}. ${normCf(Number(_ctx.cf)) < v ? `Formula SUBESTIMEAZA (Cf/11 < Cp): respondentii percep mai multa claritate decat prezice formula. Factor = ×${(v / normCf(Number(_ctx.cf))).toFixed(2)}. Posibila explicatie: factori pozitivi nemasurati (brand, experienta anterioara, context).` : normCf(Number(_ctx.cf)) > v ? `Formula SUPRAESTIMEAZA (Cf/11 > Cp): respondentii percep mai putina claritate decat prezice formula. Posibila explicatie: bariere nemasurate (confuzie, distractori, asteptari ridicate).` : `Formula prezice EXACT — aliniere perfecta intre model si realitate.`}` : ""}` },
                { heading: "Concluzie", text: `Scorul C perceput de ${val} (zona ${getZoneCp(v)}) este masura reala a claritatii mesajului si cel mai bun predictor al actiunii (CTA). ${_ctx.cf ? `Comparat cu Cf normalizat = ${normCf(Number(_ctx.cf))}, Delta = ${_ctx.delta} indica ${Number(_ctx.delta) < 1.5 ? "o buna aliniere — formula RIFC prezice corect claritatea reala." : "o discrepanta care sugereaza factori nemasurati in modelul actual."}` : ""}` },
              ]};
              case "score_delta": return { sections: [
                { heading: "Ce reprezinta Delta", text: `Delta = ${val} este diferenta absoluta intre C formula (normalizat la scala 1-10) si C perceput (scala 1-10). Masoara cat de precisa este predictia formulei RIFC — cu cat Delta este mai mica, cu atat formula prezice mai exact claritatea mesajului.` },
                { heading: "Cum se calculeaza", text: `Mai intai, Cf se normalizeaza: Cf_norm = Cf / 11 = ${_ctx.cf || "?"} / 11 = ${_ctx.cf ? normCf(Number(_ctx.cf)) : "?"}. Apoi Delta = |Cf_norm - Cp| = |${_ctx.cf ? normCf(Number(_ctx.cf)) : "?"} - ${_ctx.cp || "?"}| = ${val}. Pe scala 0-10, o Delta mica (< 1) indica predictie excelenta.` },
                { heading: "Directia erorii", text: `${_ctx.cf && _ctx.cp ? (normCf(Number(_ctx.cf)) < Number(_ctx.cp) ? `Cf/11 (${normCf(Number(_ctx.cf))}) < Cp (${_ctx.cp}): formula SUBESTIMEAZA. Respondentii percep mai multa claritate decat prezice modelul. Factor = ×${(Number(_ctx.cp) / normCf(Number(_ctx.cf))).toFixed(2)}. Cauze posibile: brand puternic, experienta anterioara pozitiva, context favorabil — factori pe care formula R+(I×F) nu ii masoara direct.` : normCf(Number(_ctx.cf)) > Number(_ctx.cp) ? `Cf/11 (${normCf(Number(_ctx.cf))}) > Cp (${_ctx.cp}): formula SUPRAESTIMEAZA. Materialul pare bun pe hartie (R, I, F ridicate) dar nu convinge in practica. Cauze posibile: bariere cognitive, distractori, asteptari ridicate ale audientei.` : `Cf/11 = Cp: aliniere perfecta.`) : ""}` },
                { heading: "Cum se interpreteaza", text: `Delta 0-1: Predictie excelenta — formula este calibrata corect. Delta 1-2: Predictie buna — variatie acceptabila. Delta 2-3: Predictie moderata — necesita investigare. Delta > 3: Discrepanta semnificativa — formula subestimeaza sistematic (Delta pozitiv). Aceasta nu inseamna ca formula "nu functioneaza" — ci ca oamenii percep mai multa claritate decat prezice modelul, din cauza factorilor nemasurati (brand, experienta, context).` },
                { heading: "Implicatii pentru CTA", text: `Delta masoara cat de bine prezice formula claritatea reala (C perceput). Claritatea este ceea ce determina actiunea (CTA): daca mesajul e clar, cumparatorul actioneaza. O Delta mica inseamna ca formula RIFC este un instrument fiabil de predictie a comportamentului (CTA). O Delta mare inseamna ca exista factori nemasurati care influenteaza decizia de cumparare dincolo de R, I si F — posibil Brand (testat in H3) sau alti moderatori.` },
                { heading: "Concluzie", text: `O Delta de ${val} ${v < 1.5 ? "sustine validitatea formulei RIFC ca predictor al claritatii si, implicit, al comportamentului (CTA)." : v < 2.5 ? "sugereaza ca formula necesita calibrare fina, dar conceptul fundamental — ca R+(I×F) prezice claritatea — este viabil." : "indica nevoia de a integra factori suplimentari (ex: Brand, experienta anterioara) in model."}` },
              ]};
              ...
              case "gate": return { sections: [
                { heading: "Ce reprezinta Relevance Gate", text: `Relevance Gate arata ca ${_ctx.pass || 0} din ${_ctx.total || 0} materiale (${val}%) au R >= ${GATE}. R este cheia de contact: sub Gate, audienta se dezangajeaza si CTA scade ~58% (de la ~6.55 la ~2.75), indiferent de calitatea I×F. Nu e ca "formula nu functioneaza" — I×F tot produce un scor — ci ca fara relevanta suficienta, motorul I×F merge in gol.` },
                ...
                { heading: "Legatura cu CTA", text: `Gate-ul este direct legat de CTA: sub R < ${GATE}, CTA scade ~58% — nu la zero, ci la un nivel insuficient pentru conversie (~2.75 vs ~6.55). Lantul cauzal: R activeaza → I×F produce C (Claritate) → C coreleaza cu CTA (H2) → Brand modereaza C→CTA (H3). Gate-ul protejeaza contra "falsei performante" — un material poate avea I×F ridicat dar sa nu genereze actiune fara relevanta suficienta.` },
              ...
              case "h1": {
                ...
                return { sections: [
                  { heading: "Ce testeaza H1 — Poarta Relevantei", text: `H1 investigheaza daca R functioneaza ca o cheie de contact (conditie de activare) in formula RIFC R+(IxF)=C. Ipoteza: sub R < ${GATE}, audienta se dezangajeaza — CTA scade ~58% (de la ~6.55 la ~2.75), indiferent de I×F. IMPORTANT: Cp nu scade la zero sub Gate (ramane ~${_ctx.cpBelow || "?"}), dar CTA scade dramatic. R nu opreste formula matematic — opreste audienta din a actiona.` },
          ...
          const grandIxF = Math.round((grandCf - grandR) * 100) / 100;
          const grandCfNorm = Math.round((grandCf / 11) * 100) / 100;
            ...
            <div style={{ position: "relative" }}>
              ...
              {interpSubTab === "total" && (
                <div>
                  ...
                  {(() => {
                    const scoreCtx = { r: grandR, i: grandI, f: grandF, cf: grandCf, cp: grandCp, delta: grandDelta, ixf: grandIxF, cfNorm: grandCfNorm };
                    const scoreCards: { label: string; value: number; color: string; k: string; scale: string; max: number; sub?: string; badge?: { text: string; color: string } }[] = [
                      { label: "R (Relevanta)", value: grandR, color: "#DC2626", k: "score_r", scale: "/10", max: 10, badge: grandR >= GATE ? { text: "GATE PASS", color: "#059669" } : { text: "GATE FAIL", color: "#DC2626" } },
                      { label: "I (Interes)", value: grandI, color: "#D97706", k: "score_i", scale: "/10", max: 10 },
                      { label: "F (Forma)", value: grandF, color: "#7C3AED", k: "score_f", scale: "/10", max: 10 },
                      { label: "I×F (Sinergia)", value: grandIxF, color: "#8B5CF6", k: "score_ixf", scale: "/100", max: 100 },
                      { label: "C formula", value: grandCf, color: "#2563EB", k: "score_cf", scale: "/110", max: 110, sub: `= ${grandCfNorm} /10` },
                      { label: "C perceput", value: grandCp, color: "#059669", k: "score_cp", scale: "/10", max: 10 },
                      { label: "Delta |Cf-Cp|", value: grandDelta, color: "#6B7280", k: "score_delta", scale: "/10", max: 10, sub: grandCfNorm < grandCp ? "Subestimeaza" : grandCfNorm > grandCp ? "Supraestimeaza" : "Exact" },
                    ];
                    return (
                      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(155px, 1fr))", gap: 10, marginBottom: 20 }}>
                        {scoreCards.map(c => {
                          const pct = Math.min((c.value / c.max) * 100, 100);
                          const barColor = c.k === "score_delta" ? (c.value < 1.5 ? "#059669" : c.value < 2.5 ? "#D97706" : "#DC2626") : c.color;
                          return (
                            <div key={c.label} style={{ ...S.configItem, textAlign: "center" as const, position: "relative" as const }}>
                              {c.badge && (
                                <div style={{ position: "absolute" as const, top: 6, right: 6, fontSize: 8, fontWeight: 700, padding: "1px 5px", borderRadius: 3, background: c.badge.color, color: "#fff", letterSpacing: 0.5 }}>{c.badge.text}</div>
                              )}
                              ...
                              <div style={{ fontSize: 22, fontWeight: 800, color: c.color }}>
                                {c.value}<span style={{ fontSize: 11, fontWeight: 500, color: "#9CA3AF" }}>{c.scale}</span>
                              </div>
                              {c.sub && <div style={{ fontSize: 10, color: "#6B7280", marginTop: 1 }}>{c.sub}</div>}
                              {/* Mini progress bar */}
                              <div style={{ margin: "6px auto 0", width: "80%", height: 4, borderRadius: 2, background: "#e5e7eb" }}>
                                <div style={{ width: `${c.k === "score_delta" ? Math.min((c.value / 5) * 100, 100) : pct}%`, height: "100%", borderRadius: 2, background: barColor, transition: "width 0.3s" }} />
                              </div>
                              <div style={{ marginTop: 6 }}>
                                <InterpBtn k={c.k} title={c.label} val={String(c.value)} ctx={scoreCtx} />
                              </div>
                            </div>
                          );
                        })}
                    ...
                    return (
                      <div style={{ background: `${getValidationColor(grandHypPct)}08`, border: `1px solid ${getValidationColor(grandHypPct)}30`, borderRadius: 10, padding: 16 }}>
                        ...
                        <div style={{ fontSize: 12, color: "#374151", lineHeight: 1.6, marginBottom: 12 }}>
                          Formula <strong>R+(I&times;F)=C</strong> este <strong>{getValidationLabel(grandHypPct).toLowerCase()}</strong> ({grandHypPct}%). Diferenta de {_gapPct}% fata de 100% se explica prin 4 reguli derivate din analiza H1:
                        </div>
                        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
                          {/* Rule 1 */}
