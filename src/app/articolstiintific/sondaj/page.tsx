          ...
          const getInterpretation = (key: string, val: string, ctx?: Record<string, unknown>): { sections: { heading: string; text: string }[] } => {
            ...
            switch (key) {
              ...
              case "score_r": return { sections: [
                ...
                { heading: "Cum se aplica in formula", text: `In formula R+(I×F)=C, R este componenta aditionala de baza. Relevanta actioneaza ca un "gate" (prag): daca R < ${GATE}, formula prezice ca materialul va avea impact scazut, indiferent cat de interesant sau bine executat este. R se aduna direct la produsul I×F. Exemplu: R = ${val} + I×F = ${_ctx.cf && _ctx.r ? (Number(_ctx.cf) - Number(_ctx.r)).toFixed(1) : "?"} → C = ${_ctx.cf || "?"}.` },
                { heading: "Relevance Gate", text: `Pragul Gate (R >= ${GATE}) este conceptul cheie al formulei RIFC: chiar daca un material e frumos (F mare) si interesant (I mare), daca NU e relevant pentru tine (R < ${GATE}), nu vei actiona. Analogie: o reclama la anvelope pentru cineva fara masina — poate fi spectaculoasa, dar irelevanta. Cu R = ${val}, ${v >= GATE ? `materialul TRECE Gate-ul — relevanta este suficienta pentru ca formula sa functioneze.` : `materialul NU trece Gate-ul — prioritatea absoluta este imbunatatirea relevantei.`}` },
                ...
                { heading: "Legatura cu Claritatea (C) si CTA", text: `R contribuie direct la C (Claritate). Un mesaj relevant este mai usor de inteles — respondentul recunoaste problema/nevoia si intelege solutia propusa. C = cat de clar e mesajul → daca e clar, actionezi (CTA: cumparare, click, inscriere). R ridicat → mesaj inteles → claritate → actiune. R scazut → mesaj ignorat → indiferent de I si F. Cu R = ${val}, contributia la claritate este ${v >= 7 ? "puternica — mesajul este relevant si usor de inteles" : v >= 5 ? "moderata — mesajul rezoneza partial" : "limitata — mesajul nu rezoneza cu audienta"}.` },
              ...
              case "score_i": return { sections: [
                ...
                { heading: "Legatura cu Claritatea (C) si CTA", text: `Interesul (I) amplifica Forma (F) pentru a genera Claritate (C). C = cat de clar este mesajul pentru respondent. Daca mesajul e clar, respondentul actioneaza (CTA — Call to Action). Deci I ridicat → I×F mare → C mare → probabilitate mai mare de actiune (cumparare, click, inscriere). Cu I = ${val}, contributia la claritate este ${v >= 7 ? "semnificativa" : v >= 5 ? "moderata" : "limitata"}.` },
              ...
              case "score_f": return { sections: [
                ...
                { heading: "Legatura cu Claritatea (C) si CTA", text: `Forma (F) amplifica Interesul (I) in ecuatia I×F. Un design profesional face mesajul mai usor de inteles (Claritate mai mare), ceea ce creste probabilitatea de actiune (CTA). F ridicat → I×F mare → C mare → respondentul intelege clar ce i se comunica si actioneaza. Cu F = ${val}, amplificarea executiei este ${v >= 7 ? "puternica — designul profesional sustine claritatea mesajului" : v >= 5 ? "moderata — executia nu obstructioneaza dar nici nu adauga claritate suplimentara" : "slaba — designul deficitar poate confuza mesajul chiar daca continutul e bun"}.` },
              ...
              case "score_ixf": return { sections: [
                { heading: "Ce reprezinta I×F (Sinergia)", text: `Scorul I×F = ${val} este produsul multiplicativ al Interesului (I) si Formei (F) — componenta de amplificare din formula RIFC. Cu I = ${_ctx.i || "?"} si F = ${_ctx.f || "?"}, sinergia I×F = ${val}. Scala este 0-100 (I max 10 × F max 10). Aceasta valoare se adauga la R pentru a obtine C formula.` },
