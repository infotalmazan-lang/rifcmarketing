...
.sidebar-header .logo { display:flex; align-items:center; }
...
.sidebar-articol { display:flex; align-items:center; gap:8px; margin:auto 16px 8px; padding:12px 16px; border-radius:12px; background:linear-gradient(135deg, #059669, #047857); color:#fff; font-size:12px; font-weight:700; letter-spacing:.5px; text-decoration:none; cursor:pointer; transition:all .2s; border:none; box-shadow:0 4px 12px rgba(5,150,105,0.25); }
.sidebar-articol:hover { transform:translateY(-2px); box-shadow:0 6px 16px rgba(5,150,105,0.3); }
.sidebar-sondaj { display:flex; align-items:center; gap:8px; margin:0 16px 16px; padding:12px 16px; border-radius:12px; background:linear-gradient(135deg, #DC2626, #B91C1C); color:#fff; font-size:12px; font-weight:700; letter-spacing:.5px; text-decoration:none; cursor:pointer; transition:all .2s; border:none; box-shadow:0 4px 12px rgba(220,38,38,0.25); }
...
+'<tr><td class="mono">STM-002</td><td>CloudMetric Landing</td><td>Site</td><td>SaaS B2B</td><td class="mono" style="color:var(--green)">8</td><td class="mono">7</td><td class="mono">9</td><td class="mono">71</td><td><span class="tag" style="background:var(--amber-dim);color:var(--amber)">PILOT</span></td></tr>'
...
+'<tr><td class="mono">STM-015</td><td>FinPro Pitch Deck</td><td>RadioTV</td><td>FinTech</td><td class="mono" style="color:var(--green)">8</td><td class="mono">9</td><td class="mono">8</td><td class="mono">80</td><td><span class="tag" style="background:var(--blue-dim);color:var(--blue)">NOU</span></td></tr>'
...
+'<tr><td class="mono">STM-030</td><td>SoundWave Radio Spot</td><td>Influencer</td><td>Music</td><td class="mono">5</td><td class="mono">8</td><td class="mono">7</td><td class="mono">61</td><td><span class="tag" style="background:var(--blue-dim);color:var(--blue)">NOU</span></td></tr>'
+'</table>'
+'<div class="ex-note">Afisati 10 din 30 stimuli. Distributie: Site(3), Social(3), Email(3), Google(3), Video(3), Billboard(3), Ambalaj(3), SMS(3), RadioTV(3), Influencer(3). Anonimizati. Randomizare per respondent.</div>'
...
+'<div class="ex-card"><div class="ex-card-title">Instrument</div><div class="ex-card-body"><strong>Platforma:</strong> rifcmarketing.com/articolstiintific/sondaj<br>Wizard cu auto-save, resume din localStorage, mobile-first. Durata: ~8 min.</div></div>'
+'</div>'
+'</div></div>',
...
+'<tr><td class="mono">C-003</td><td>CodeNest Rebrand</td><td>Site</td><td class="mono">55</td><td class="mono">1.5%</td><td class="mono">1.2%</td><td class="mono">\u20AC45</td><td class="mono">1.8x</td></tr>'
...
function App() {
  ...
  /* Detect if URL is /articolstiintific/sondaj or has #sondaj — auto-open sondaj view */
  var parentUrl = '';
  var parentHash = '';
  try { parentUrl = window.parent.location.pathname; parentHash = window.parent.location.hash; } catch(e) {}
  var activeStage = (parentUrl.indexOf('/sondaj') !== -1 || parentHash === '#sondaj') ? 'sondaj' : 'overview';
  /* Clean up hash — replace URL with clean /articolstiintific/sondaj */
  if (parentHash === '#sondaj') { try { window.parent.history.replaceState({stage:'sondaj'}, '', '/articolstiintific/sondaj'); } catch(e) {} }
  ...
  function bindEvents() {
    ...
    var articolBtn = document.querySelector('[data-articol]');
    if(articolBtn) { articolBtn.addEventListener('click', function() {
      activeStage = 'articol'; activeTask = null;
      /* Update URL to /articolstiintific/osf */
      try { window.parent.history.pushState({stage: 'articol'}, '', '/articolstiintific/osf'); } catch(e) {}
      render();
    }); }
    ...
    if(sondajBtn) { sondajBtn.addEventListener('click', function() {
      ...
      /* Update URL to /articolstiintific/sondaj */
      try { window.parent.history.pushState({stage: 'sondaj'}, '', '/articolstiintific/sondaj'); } catch(e) {}
      render();
    }); }
    ...
    document.querySelectorAll('[data-action]').forEach(function(el) {
      el.addEventListener('click', function(e) {
        ...
        if (action === 'copySurveyLink') {
          var surveyUrl = window.location.origin + '/articolstiintific/sondaj';
