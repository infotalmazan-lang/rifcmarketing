...
ALTER TABLE public.survey_experts
  ...
  ADD COLUMN IF NOT EXISTS marketing_roles text[] DEFAULT '{}';`;
    ...
    if (migration === "023" || migration === 23) {
      // Migration 023: Predictive validation tables (Case Study)
      const { error: eComp } = await supabase.from("predictive_companies").select("id").limit(1);

      if (eComp) {
        const sql = `
CREATE TABLE IF NOT EXISTS public.predictive_companies (
  id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  name TEXT NOT NULL, industry TEXT NOT NULL DEFAULT 'Altele',
...
CREATE TABLE IF NOT EXISTS public.predictive_campaigns (
  ...
  name TEXT NOT NULL, channel TEXT NOT NULL, campaign_type TEXT NOT NULL,
  period_start DATE, period_end DATE, budget_eur NUMERIC,
  creative_image_url TEXT, creative_link TEXT, admin_notes TEXT,
  paired_campaign_id UUID,
  eval1_token TEXT UNIQUE, eval2_token TEXT UNIQUE,
  is_active BOOLEAN NOT NULL DEFAULT TRUE,
  created_at TIMESTAMPTZ NOT NULL DEFAULT NOW()
);
...
CREATE TABLE IF NOT EXISTS public.predictive_evaluations (
  ...
  evaluator_name TEXT, evaluator_email TEXT, evaluator_experience TEXT,
  ...
  comment_r TEXT, comment_i TEXT, comment_f TEXT, comment_cta TEXT,
  ...
  time_spent_scoring_sec INTEGER, time_spent_kpi_sec INTEGER,
...
CREATE TABLE IF NOT EXISTS public.predictive_kpis (
  ...
  kpi_name TEXT NOT NULL, kpi_value NUMERIC NOT NULL,
  kpi_unit TEXT NOT NULL DEFAULT '', data_source TEXT, notes TEXT,
  created_at TIMESTAMPTZ NOT NULL DEFAULT NOW()
);
...
END $$;`;
        ...
        return NextResponse.json({ migrated: true, message: "Migration 023 applied. Predictive validation tables created." });
      }

      return NextResponse.json({ migrated: true, message: "Migration 023 already applied. Predictive tables exist." });
    }
